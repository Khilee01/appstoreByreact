...
class AppStore extends Component {
  ...
  render() {
    const {activeTab, searchappName} = this.state
    const categorialtAppsList = appsList.filter(
      eachApp => eachApp.category === activeTab,
    )

    const resultAppList = categorialtAppsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchappName.toLowerCase()),
    )
    return (
        ...
        <ul className="ultab">
          {tabsList.map(eachTabDetails => (
            <TabItem
              ...
              isActive={activeTab === eachTabDetails.tabId}
